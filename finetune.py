...
from distillm import forward_kl, reverse_kl, js_distance, tv_distance, skewed_forward_kl, skewed_reverse_kl
    ...
    if args.model_parallel:
        ...
        try:
            model = AutoModelForCausalLM.from_pretrained(
                ...
                config=config,
                device_map={"": device},
                torch_dtype=torch.bfloat16 if args.model_type == "qwen" else torch.float16
            )
        except Exception as e:
            print(f"Error loading with mixed precision: {e}. Falling back to float32.")
            model = AutoModelForCausalLM.from_pretrained(
                args.teacher_model_path,
                config=config,
                device_map={"": device},
...
def get_learning_rate_scheduler(args, optimizer):
    ...
    if args.lr_decay_style == "constant":
        ...
        lr_scheduler = get_polynomial_decay_schedule_with_warmup(
            optimizer, num_warmup_steps=args.warmup_iters, num_training_steps=args.total_iters, power=0.5)
    else:
...
def finetune(args, tokenizer: AutoTokenizer, model: deepspeed.DeepSpeedEngine, optimizer: AdamW, lr_scheduler, dataset, device, teacher_model=None):
    ...
    adaptive_threshold = args.init_threshold if "adaptive" in args.type else 0.5  # Default is set to 0.5
